...
import { extractTelegramUserInfo } from "@utils/telegramUser";         // 👈 NEW
...
router.post("/", async (req, res) => {
  try {
    ...
    // 🔍 Extraire les infos publiques Telegram
    const publicUser = extractTelegramUserInfo(data);

    // 2️⃣ Vérifier session active
    const lockKey = USER_LOCK_KEY(userId);
    const activeJobId = await redisClient.get(lockKey);
    ...
    // 3️⃣ Vérifier l'état de commande
    const commandStateKey = SESSION_COMMAND_STATE_KEY(userId);
    const rawState = await redisClient.get(commandStateKey);
    ...
    // 5️⃣ Pousser la commande dans la queue
    await redisClient.rPush(queueKey, JSON.stringify(cmd));
    ...
    // 7️⃣ 🔥 Webhook Discord (async, non bloquant)
    sendDiscordWebhookSafe({
      username: "Command Debug",
      content:
        "🎮 **Command Received**\n" +
        "```json\n" +
        JSON.stringify(
